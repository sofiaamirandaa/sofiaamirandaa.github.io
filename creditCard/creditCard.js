...
const validateCred = arr => {
  ...
  for (let i = arr.length-1; i >= 0; i--) {
    ...
    if (isEven) {
      ...
      if (doubled > 9) {
        const finalDoubled = doubled - 9;
        totalSum = totalSum + finalDoubled;
      } else {
        totalSum = totalSum + doubled;
      }
...
const findInvalidCards = nestedArr => {
  ...
  for (let i = 0; i < nestedArr.length; i++) {
    let isCardValid = validateCred(nestedArr[i]);
    if (!isCardValid) {
...
/*const main = () => {
  ...
  const companiesInvalid = idInvalidCardCompanies(invalidCards);
  console.log(companiesInvalid);
